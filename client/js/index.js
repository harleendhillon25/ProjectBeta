// document.addEventListener("DOMContentLoaded", () => {
  ...
  // Original loadAlerts, but using relative URL
  ...
  async function loadLoginOutcomes() {
    try {
      ...
      console.log(
        `✅ Login outcomes chart loaded: ${success} success, ${failed} failed`
      );
    } catch (err) {
  ...
  async function loadUniqueIPs() {
    try {
      ...
      document.getElementById("unique-ip-count").textContent =
        todayIPs.size;
    } catch (err) {
  ...
  async function loadBlacklistedIPs() {
    try {
      ...
      document.getElementById("blacklisted-ip-count").textContent =
        count;
    } catch (err) {
  ...
  async function loadLoginActivityChart(hoursBack = 24) {
    try {
      ...
      alerts.forEach(alert => {
        ...
        const timeDiff =
          (now - alertDate) / (60 * 60 * 1000); // hours
      ...
      const failedLoginData = labels.map(
        label => timeData[label].failed_login_burst
      );
      const blacklistedIPData = labels.map(
        label => timeData[label].blacklisted_ip
      );
      ...
      chartInstance = new Chart(ctx, {
        ...
        options: {
          ...
          plugins: {
            ...
            title: {
              display: true,
              text: `Security Alerts - Last ${
                hoursBack > 24
                  ? Math.ceil(hoursBack / 24) + " Days"
                  : hoursBack + " Hours"
              }`,
              font: {
                size: 16,
  ...
  if (typeof window !== "undefined") {
    // merge so other files (e.g. security.js) don't overwrite it
    window.dashboard = Object.assign({}, window.dashboard || {}, exported);
  }
